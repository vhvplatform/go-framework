...
start "Auth Service" cmd /k "cd /d e:\NewFrameWork\go-auth-service\server && go run cmd/main.go"
timeout /t 3 /nobreak >nul

REM Start Tenant Service
echo [2/5] Starting Tenant Service...
start "Tenant Service" cmd /k "cd /d e:\NewFrameWork\go-tenant-service\server && go run cmd/tenant/main.go"
timeout /t 3 /nobreak >nul

REM Start User Service
echo [3/5] Starting User Service...
start "User Service" cmd /k "cd /d e:\NewFrameWork\go-user-service\server && go run cmd/main.go"
timeout /t 3 /nobreak >nul

REM Start API Gateway
echo [4/5] Starting API Gateway...
start "API Gateway" cmd /k "cd /d e:\NewFrameWork\go-api-gateway\server && go run cmd/main.go"
timeout /t 3 /nobreak >nul

REM Start React Frontend
echo [5/5] Starting React Frontend...
start "React Frontend" cmd /k "cd /d e:\NewFrameWork\go-user-service\client && pnpm install && pnpm dev"
